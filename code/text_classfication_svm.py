...
vect = CountVectorizer(min_df=2, max_df=0.95, max_features=i_max_features_cnt, strip_accents='unicode', analyzer='word', token_pattern=r'\w{1,}', ngram_range=(1, i_ngram_range_upbound))
...
#clf = linear_model.SGDClassifier(shuffle=True, loss='hinge', penalty = 'l2', random_state=0, early_stopping=True, validation_fraction=0.2, n_iter_no_change=10, max_iter=1000, tol=1e-04)
...
model_search_LR = GridSearchCV(clf, param_grid = params, scoring = 'roc_auc', cv = 2, verbose = 10, error_score='raise', iid=True, n_jobs=2)
...
if b_do_model_selection:
    ...
    if b_do_cross_validation:
        ...
        cv_results = cross_validate(clf, X_train_normalized, Y_train_whole, scoring='f1_weighted', cv=i_cross_validation_fold_count, n_jobs=2, return_train_score=False, verbose=10)
